...
const TodoModal: React.FC<TodoModalProps> = ({ visible, onClose, onSaved, editTodo }) => {
    ...
    useEffect(() => {
        if (visible) {
            ...
            if (editTodo) {
                ...
                setFormData({
                    recipient_id: String(editTodo.recipient_id),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEditing && editTodo) {
                ...
                response = await todoService.update(editTodo.id, {
                ...
                // Create new todo
                response = await todoService.create({
                    sender_id: user!.id,
            ...
            if (response.success) {
                toast.success(isEditing ? 'Todo aggiornato' : 'Todo creato');
                onSaved();
                onClose();
            } else {
                toast.error(isEditing ? 'Errore nell\'aggiornamento' : 'Errore nella creazione');
            }
        } catch (error) {
            toast.error(isEditing ? 'Errore nell\'aggiornamento' : 'Errore nella creazione');
        } finally {
    ...
    return (
        <CModal visible={visible} onClose={onClose} size="lg">
            <CModalHeader>
                <CModalTitle>{isEditing ? 'Modifica Todo' : 'Nuovo Todo'}</CModalTitle>
            </CModalHeader>
            <CForm onSubmit={handleSubmit}>
                ...
                <CModalFooter>
                    ...
                    <CButton color="primary" type="submit" disabled={saving}>
                        {saving ? <CSpinner size="sm" /> : (isEditing ? 'Salva' : 'Crea Todo')}
                    </CButton>
                </CModalFooter>
            </CForm>
        </CModal>
