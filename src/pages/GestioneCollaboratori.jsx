...
import { cilUserPlus, cilCheck, cilX, cilReload, cilPeople } from '@coreui/icons';
import apiClient from '../api/client';

const GestioneCollaboratori = () => {
  const [collaboratori, setCollaboratori] = useState([]);
  const [nuoviCandidati, setNuoviCandidati] = useState([]);
  ...
  const [selezioneCollaboratori, setSelezioneCollaboratori] = useState({});
  const [tipiAssegnati, setTipiAssegnati] = useState({});
  ...
  const caricaCollaboratori = async () => {
    try {
      ...
      const response = await apiClient.get('/collaboratori/');
      ...
      if (response.data.success) {
        ...
        // Pre-seleziona tutti i collaboratori (confermati + candidati)
      ...
      setError('Errore nel caricamento collaboratori: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const caricaStatistiche = async () => {
    try {
      const response = await apiClient.get('/collaboratori/statistiche');
      if (response.data.success) {
  ...
  const salvaSelezione = async () => {
    try {
      ...
      const tipiPerSalvataggio = {};
      codiciSelezionati.forEach(codice => {
        tipiPerSalvataggio[codice] = tipiAssegnati[codice];
      });
      
      const response = await apiClient.post('/collaboratori/salva-selezione', {
      ...
      setError('Errore nel salvare: ' + err.message);
    } finally {
      setSaving(false);
    }
  };

  const toggleSelezione = (codice) => {
  ...
  const aggiornaeTipo = (codice, nuovoTipo) => {
                      ...
                      >
                        ...
                        <div className="d-flex align-items-center">
                          <CFormSelect
                            ...
                            onChange={(e) => aggiornaeTipo(collab.codice_fornitore, e.target.value)}
                      ...
                      >
                        ...
                        <div className="d-flex align-items-center">
                          <CFormSelect
                            ...
                            onChange={(e) => aggiornaeTipo(candidato.codice_fornitore, e.target.value)}
                            style={{width: '120px', marginRight: '10px'}}
                          >
                            {tipiCollaboratore.map(tipo => (
                              <option key={tipo} value={tipo}>{tipo}</option>
