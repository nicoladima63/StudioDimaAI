...
class TemplateManager:
    ...
    def preview_template(self, name: str, data: Dict[str, Any] = None, custom_content: Optional[str] = None) -> Dict[str, Any]:
        """Genera un'anteprima di un template."""
        content_to_render = custom_content
        if not content_to_render:
            template = self.get_template_by_name(name)
            if not template:
                return {'success': False, 'message': f"Template '{name}' non trovato."}
            content_to_render = template['content']
        
        try:
            rendered_message = content_to_render.format(**(data or {})) # Corrected line
            return {
                'success': True,
                'message': rendered_message,
                'length': len(rendered_message),
                'estimated_sms_parts': (len(rendered_message) // 160) + 1 # Stima base
            }
        except KeyError as e:
            return {'success': False, 'message': f"Variabile mancante nel template: {e}"}
        except Exception as e:
            return {'success': False, 'message': f"Errore durante l'anteprima: {e}"}
