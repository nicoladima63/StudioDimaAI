...
class AutomationService(BaseService):
    ...
    def _enrich_context_with_patient_data(self, context: Dict[str, Any], rule_id: int, action_name: str) -> Dict[str, Any]:
        from services.dbf_data_service import get_dbf_data_service
        dbf_data_service = get_dbf_data_service()

        logger.debug(f"[Rule {rule_id}] _enrich_context_with_patient_data: Initial context for enrichment: {context}")

        patient_id_key = COLONNE['appuntamenti']['id_paziente']
        patient_id = context.get(patient_id_key) # DB_APPACOD
        logger.debug(f"[Rule {rule_id}] _enrich_context_with_patient_data: Valore di {patient_id_key} (DB_APPACOD): '{patient_id}'")

        if not patient_id:
            logger.info(f"[Rule {rule_id}] _enrich_context_with_patient_data: ID paziente non trovato. Tentativo di estrazione nome e telefono da DB_APDESCR e DB_NOTE.")

            patient_name_key = COLONNE['appuntamenti']['descrizione']
            patient_name = context.get(patient_name_key, '').strip() # DB_APDESCR
            logger.debug(f"[Rule {rule_id}] _enrich_context_with_patient_data: Valore di {patient_name_key} (DB_APDESCR): '{patient_name}'")
            ...
            note_key = COLONNE['appuntamenti']['note']
            note_content = context.get(note_key, '') # DB_NOTE
            logger.debug(f"[Rule {rule_id}] _enrich_context_with_patient_data: Valore di {note_key} (DB_NOTE): '{note_content}'")

            import re # Ensure re is imported for this function
            phone_match = re.search(r'\d{10}', note_content)
            if not phone_match:
                raise ValidationError(f"Azione '{action_name}' fallita: Numero di telefono di 10 cifre non trovato in DB_NOTE: '{note_content}'.")
            extracted_phone = phone_match.group(0)
            context['telefono'] = extracted_phone
            logger.info(f"[Rule {rule_id}] _enrich_context_with_patient_data: Dati paziente (nuovo) estratti: Nome='{patient_name}', Telefono='{extracted_phone}'.")

        else:
            logger.info(f"[Rule {rule_id}] _enrich_context_with_patient_data: ID paziente '{patient_id}' trovato. Recupero dati dal DB.")
            patient_data = dbf_data_service.get_patient_by_id(patient_id)
            if not patient_data:
                raise ValidationError(f"Azione '{action_name}' fallita: dati paziente non trovati per ID '{patient_id}'.")

            context.update(patient_data)
            logger.debug(f"[Rule {rule_id}] _enrich_context_with_patient_data: Dati paziente da DB: {patient_data}")
            ...
            context['nome_completo'] = patient_data.get(COLONNE['pazienti']['nome'], '')
            context['telefono'] = extracted_phone
            logger.info(f"[Rule {rule_id}] _enrich_context_with_patient_data: Dati paziente (esistente) recuperati: Nome='{context['nome_completo']}', Telefono='{extracted_phone}'.")
