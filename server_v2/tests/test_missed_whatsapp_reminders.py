...
def test_recovery_falls_back_to_sms_when_no_whatsapp(monkeypatch):
    logged = []
    ...
    monkeypatch.setattr(reminders, 'send_sms_reminder', lambda *args: {'success': True, 'message_id': 's1', 'channel': 'sms'})
    monkeypatch.setattr(reminders, '_log_communication', lambda *args: logged.append(args) or 1)
    monkeypatch.setattr(reminders, '_write_log', lambda *args: None)
    ...
    assert stats['sent_sms'] == 1
    assert stats['skipped_no_whatsapp'] == []
    assert logged[0][3] == 'sms'
