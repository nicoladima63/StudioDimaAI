# server/app/incassi/controller.py
from flask import Blueprint, jsonify, request
from datetime import datetime
from flask_jwt_extended import jwt_required
from server.app.core.db_handler import DBHandler
from server.app.incassi.service import IncassiService
...
def get_incassi_by_date():
    try:
        ...
        mese = request.args.get('mese')  # può essere None
        if anno is None:
...
def get_incassi_by_periodo():
    try:
        anno = request.args.get('anno')
        tipo = request.args.get('tipo')  # 'mese', 'trimestre', 'quadrimestre'
