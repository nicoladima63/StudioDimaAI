...
from scripts.test_tools import test_due_recalls
...
class GuiApp(tk.Tk):
    ...
    def restart_app(self):
        risposta = messagebox.askyesno("Conferma", "Vuoi davvero riavviare l'app?")
        if risposta:
            logging.info("Riavvio applicazione...")
    ...
    def sync_calendar(self):
        ...
        try:
            if not self.calendar_sync:
                self.calendar_sync = GoogleCalendarSync(self.db_handler)
    ...
    def create_environment_frame(self):
        ...
        ttk.Radiobutton(env_frame, 
                ...
                #Riavvio app
        btn_restart = tk.Button(self, text="🔄 Riavvia App", command=self.restart_app)
        ...
        try:
            ...
            def background_delete():
                try:
                    ...
                    self.after(0, lambda: self.calendar_result.insert(tk.END, f"\nEliminati {deleted} eventi dal calendario {calendar_name}\n"))
                    self.after(0, lambda: messagebox.showinfo("Completato", f"Eliminati {deleted} eventi dal calendario"))
                except Exception as e:
                    self.after(0, lambda: messagebox.showerror("Errore", f"Errore durante la pulizia del calendario: {str(e)}"))
                    logging.error(f"Errore durante la pulizia del calendario: {str(e)}")
                finally:
                    self.after(0, self.progress.stop)
                    self.after(0, self.progress.pack_forget)
            ...
            Thread(target=background_delete).start()
        ...
        try:
            ...
            for app in to_create:
                try:
                    ...
                    event = self.calendar_sync.create_event(mapped, cal_id=calendar_id,sync_map=sync_map)
