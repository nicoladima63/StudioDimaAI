...
const ComparisonChart: React.FC<ComparisonChartProps> = ({ data, annoCorrente, annoPrecedente }) => {
  const chartData = data.map((m) => ({
    ...
    [annoCorrente.toString()]: m.produzione_corrente,
    [annoPrecedente.toString()]: m.produzione_precedente,
  }));
      ...
      <CCardBody>
        <ResponsiveContainer width="100%" height={350}>
          <BarChart data={chartData} margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
            ...
            <Legend />
            <Bar dataKey={annoPrecedente.toString()} name={annoPrecedente.toString()} fill="#9da5b1" radius={[4, 4, 0, 0]} />
            <Bar dataKey={annoCorrente.toString()} name={annoCorrente.toString()} fill="#321fdb" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </CCardBody>
    </CCard>
