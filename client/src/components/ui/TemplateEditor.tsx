...
import { templatesService, type Template, type TemplateValidation } from '@/api/services/templates.service';
...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ tipo, title, defaultVariables }) => {
  // State management
  ...
  const loadTemplate = async () => {
    try {
      setLoading(true);
      const templateData = await templatesService.getTemplate(tipo);
  ...
  const saveTemplate = async () => {
    try {
      setSaving(true);
      const updatedTemplate = await templatesService.updateTemplate(tipo, content, description);
  ...
  const resetTemplate = async () => {
    ...
    try {
      setSaving(true);
      const resetTemplate = await templatesService.resetTemplate(tipo);
  ...
  const generatePreview = async () => {
    try {
      setPreviewing(true);
      const previewResponse = await templatesService.previewTemplate(tipo, content);
      
      if (previewResponse.success) {
        setPreview(previewResponse.preview || '');
        setValidation(previewResponse.validation || null);
      }
    } catch (error) {
